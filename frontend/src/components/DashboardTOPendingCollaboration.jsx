...
function DashboardTOPendingCollaboration() {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://api.fyp23s424.com/getTournaments")
      .then((response) => {
  ...
  useEffect(() => {
    // Fetch tournaments when the component mounts
    axios
      .get("https://api.fyp23s424.com/getStatus")
      .then((response) => {
  ...
  const renderTournamentList = () => {
    ...
    return (
      <table className="dashboard-table" style={{ backgroundColor: "white" }}>
        <img
          className="bg"
          src={bgmImage2}
          alt="Background"
        ...
        <thead>
          <tr>
            <th style={{ backgroundColor: "orange" }}>Tournament</th>
            <th style={{ backgroundColor: "orange" }}>Details</th>
            <th style={{ backgroundColor: "orange" }}>Status</th>
            {currentTab === "Pending" && (
              <th style={{ backgroundColor: "orange" }}>Action</th>
            )}
          </tr>
        </thead>
        <tbody>
            ...
            <tr key={tournament._id}>
              ...
              <td>
                {status
                  .filter(
                    (s) =>
                      s.tournamentId === tournament._id && s.userId === user._id
                  )
                  .map((s, index) => (
